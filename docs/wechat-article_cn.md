...
为了让这次 VibeCoding 不停留在“聊得很嗨的 Demo”，我们从一开始就按“可联调、可扩展”的思路搭系统。
...
这是“装上去的大脑”：负责理解自然语言、多轮对话、工具调用（多次请求 ToDo 后端查询/写入）、以及把过程以流式方式推给前端。它的目标是“把不确定性变成可控的工程流程”。
...
我们的开发顺序也符合这个结构：  
**先把 ToDo 场景的后端 + 客户端基础架子搭好**，让接口与体验都可跑通；然后再在这个基础上，通过 `auto_agent/` 把“智能能力”加进来，并用真实接口的场景用例把它一轮轮打磨到可用。
