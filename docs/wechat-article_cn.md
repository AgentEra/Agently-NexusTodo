...
本文视角：Agently 团队运营复盘/采访记录，写给开发者、VC、关注 AI 应用的人，以及对“新开发范式”好奇的读者。

如果你最近刷到 “VibeCoding”，大概率会有一种冲动：把需求丢给模型，“一路 Accept All”，然后期待它自己把项目写完。

我们在直播里做了类似的尝试。但关注点不在“写得快不快”，而是：**怎么把这种新范式做成可复制、可交付的工程流程**。
...
为了让你读起来有一条清晰的线索，我把这次复盘按 5 个问题来讲（也是我们想强调的重点）：

> Tips（阅读路线）  
> 1）VibeCoding 的命题到底是什么？为什么一到“智能体/智能系统”就容易翻车？  
> 2）我们怎么设计与筹划（SpecDD / API 文档规范 / 场景用例）让多人多会话能并行？  
> 3）我们做成了什么样（一个能联调、能回归的最小系统）？  
> 4）难点在哪里，我们怎么解决？Agently 到底帮了多大忙？为什么能帮这么大？  
> 5）如果换成你的项目，你应该在什么问题上“照着用一次试试”？

## 先把“架子”搭起来，再把“大脑”装上去

顺着“跑起来、联起来、跑得稳”这个标准，我们团队的做法也很朴素：先把业务系统该有的骨架搭起来，再把智能能力装上去。
...
这一节把莫欣老师那句“SpecDD 很重要”展开说清楚：它到底解决什么问题？为什么它对 VibeCoding 尤其关键？
...
而当你把“智能体/智能系统”加进来，这种协作基准就更重要了：  
因为智能模块天然带着不确定性，你越早把边界、接口、场景用例写清楚，后面就越不容易陷入“解释一大堆但跑不通”的泥潭。
...
我们把这些踩坑记录沉淀进了仓库里的 skills 文档，里面甚至直接写了 “Lessons from NexusTodo”，你可以当成一份“下一次别再踩”的清单：
...
1) **你卡在“模型听不懂 / 输出不稳定”**  
典型症状：同一个需求每次输出都不一样；字段缺失；解析靠正则；一改提示词全崩。  
可以试试：先把输出变成“合同式结构”，用 Output Format + Ensure Keys 把关键字段兜住，再谈“聪明”。

2) **你卡在“多步任务做不完 / 反复兜圈”**  
典型症状：查了信息不继续写；写操作前反复澄清；循环调用工具；前端重复刷同一张卡片。  
可以试试：把流程显式化，用 TriggerFlow 把“什么时候该查、什么时候该写、什么时候该停”写成信号驱动的编排；同时用可执行场景测试去卡住回归。

3) **你卡在“能跑但不敢交付 / 联调全靠人肉”**  
典型症状：UI 看起来能用，但一联调就发现数据不一致；线上问题难复现；功能越多越不敢改。  
可以试试：把 SpecDD + API 文档规范当成第一产物，把真实接口的集成用例当成验收门槛，让“能用”变成“可回归、可迭代”。

一句话：Agently 之所以对 VibeCoding 特别“适配”，本质是它把智能体系统开发里最折磨人的不确定性，拆成了可约束、可编排、可验证的工程模块。

---

## 7) 只要对 AI 应用开发有爱，谁来都欢迎
